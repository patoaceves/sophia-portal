...
(async () => {
  ...
  renderShell({
    persona,
    title: `Resultados · ${def ? def.pilar : "Autoevaluación"}`,
  ...
  if (!def) {
    ...
    document.querySelector(".app-main").innerHTML = `
      <div class="empty-state">
        ...
        <p class="empty-state__desc">La autoevaluación solicitada no existe.</p>
      </div>`;
    return;
  }
    ...
    if (!data.tieneResultados) {
      document.querySelector(".app-main").innerHTML = `
        <div class="empty-state">
          ...
          <p class="empty-state__desc">Toma la Autoevaluación de ${escapeHtml(def.pilar)} para ver tu wedge y tu banda.</p>
          <a href="/app/cursos" class="btn btn-accent" style="margin-top:var(--s-4);">Ir a mis cursos</a>
        </div>`;
      return;
    }
    renderResultados(def, data, cursoSlug, nextLeccionId);
  } catch (e) {
    stopLoader();
    console.error("get-resultados-autoeval failed:", e);
    document.querySelector(".app-main").innerHTML = `
      <div class="empty-state">
...
function renderResultados(def, data, cursoSlug, nextLeccionId) {
  const { total, pct, banda, completedAt } = data;

  // Texto de banda: del backend si vino (compat), si no, de autoeval-defs.
  ...
  document.querySelector(".app-main").innerHTML = `
    <header class="page-header page-header--with-action">
      <div class="page-header__main">
        <span class="page-eyebrow">Tu autoevaluación de ${escapeHtml(def.pilar)} · ${escapeHtml(fecha)}</span>
        <h2 class="page-title">Tu integración de ${escapeHtml(def.pilar)}</h2>
        <p class="page-subtitle">
          Una medida de qué tan integrado tienes ${escapeHtml(def.pilar)} en tu vida,
