...
import { getRecursos } from "./recursos-defs.js";
...
function renderRecursosTab(ctx) {
  const recursos = getRecursos(ctx.slug);
  ...
  return `
    ...
    <div class="recursos-list">
      ${recursos.map(renderRecursoCard).join("")}
    </div>
  `;
}

function renderRecursoCard(r) {
  const meta = [r.tipo === "pdf" ? "PDF" : (r.tipo || "").toUpperCase(), r.idioma, r.tamano]
    .filter(Boolean)
    .join(" · ");
  return `
    <a class="recurso-card" href="${escapeHtml(r.url)}" target="_blank" rel="noopener noreferrer">
      <div class="recurso-card__icon">${icon(r.tipo === "enlace" ? "external" : "lessonDocumento")}</div>
      <div class="recurso-card__body">
        <span class="recurso-card__meta">${escapeHtml(meta)}</span>
        <h4 class="recurso-card__title">${escapeHtml(r.titulo)}</h4>
        ${r.autor ? `<div class="recurso-card__author">${escapeHtml(r.autor)}</div>` : ""}
        ${r.descripcion ? `<p class="recurso-card__desc">${escapeHtml(r.descripcion)}</p>` : ""}
      </div>
      <div class="recurso-card__action" aria-hidden="true">${icon("download")}</div>
    </a>
  `;
}
